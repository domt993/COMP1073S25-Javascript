...
btn.addEventListener('click', function() {
	...
	for (let i = 0; i < cities.length; i++) {
		if (searchName === cities[i]) {
			result.textContent = `Found ${searchName} at index ${i}.`;
			break;
		}
		else {
			result.textContent = `Sorry, ${searchName} is not in the list.`;
		}
 	}
...
while (j < cities.length) {
	/* STEP 4a: Create a WHILE loop to iterate through the cities array */
