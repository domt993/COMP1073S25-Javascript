...
function sendChatMessage() {
    ...
    }).then(res => {return res.json();}).then(json => {displayChat(json, "Assistant");})
        displayChat(json, "User");
    })
...
function displayChat(json, type) {
    console.log(json);
    if (type === "User") {
        let userPara = document.createElement("p");
        userPara.className = "chat-message";
        userPara.textContent = `User: ${userMessage.value}`;
        conversation.appendChild(userPara);
    } else if (type === "Assistant") {
        let assistantPara = document.createElement("p");
        assistantPara.className = "chat-message";
        assistantPara.textContent = `Claude: ${json.content[0].text}`;
        conversation.appendChild(assistantPara);
    }
}
